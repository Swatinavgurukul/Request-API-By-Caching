...
if childSlugList != []:
        ...
        def requestChildCon(childurl):
                ...
                with open("Ex_"+str(userInput1)+"childContent_"+str(selectChildEx)+".json","w") as file:
                        file.write(response.content)
                return response.json()
        # requestChildCon(childSlugUrl)

        def read_file(f_read):
                with open("Ex_"+str(userInput1)+"childContent_"+str(selectChildEx)+".json","r") as file:
                        data_read=file.read()
        ...
        if os.path.exists("Ex_"+str(userInput1)+"childContent_"+str(selectChildEx)+".json"):
                data_load4=read_file("Ex"+str(userInput1)+"childContent_"+str(selectChildEx)+".json")
                contentChild(data_load4)
                print "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
        else:
                requestChildCon(childSlugUrl)
                data_load4=read_file("Ex_"+str(userInput1)+"childContent_"+str(selectChildEx)+".json")
                contentChild(data_load4)
                print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
        # else:
...
for i in range(5):
        choiceExercise = raw_input("Press Next(n/N) / Back or Up (u/U) / out (o/O):- ")
        if i==len(childSlugList):
                print "There is no slug"
                break
        elif choiceExercise == "n" or choiceExercise == "N":
                childExSlug1 = childSlugList[selectChildEx+i]
                childSlugUrl2="http://saral.navgurukul.org/api/courses/"+str(userInput1-1)+"/exercise/getBySlug?slug="+childExSlug1
                requestChildCon(childSlugUrl2)
                data_load4=read_file("Ex_"+str(userInput1)+"childContent_"+str(selectChildEx+1)+".json")
                contentChild(data_load4)
                print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"       
